...
### TODO: verify root has minimal regular children when server is running
After all server-first + upgrade fixes, root should ideally have only the relay server as a direct child (or at most 1 regular peer). If root still accumulates K regular children + server during manual testing, investigate: (1) whether the batch timer reset is giving the server enough time during initial connection, (2) whether `_reviewResponses` should skip `p2pRoots` entirely when `serverFirst` is true and a server is known to exist (`_serverInfo` or `_serverUrl`), (3) whether nodes added before the relay server finishes connecting bypass server-first. Test 24 covers upgrade-skips-root; test 23 covers server-first preference; but neither explicitly asserts that root ends up with 0 regular children after a full server-first → upgrade cycle with many nodes. Consider adding a test that verifies `root.connectedDownstreamCount` (excluding server) is 0 or 1 after stabilization.
