...
### Fast server reconnect for orphaned nodes
When a P2P upstream dies and the node has cached `_serverInfo` (from a previous `_onconfig`), it can bypass the slow Firebase request/response cycle entirely. `_connectToServerDirect()` creates a `ServerTransport` directly using the cached `serverUrl`, wires it as the primary upstream (with notification channel, heartbeat, mask handling), and starts the P2P upgrade timer. This is triggered in `_onupstreamDisconnect` when `peer.didConnect && _serverInfo && serverFirst` — meaning the node had a working connection, knows about the server, and is configured for server-first. The normal `connect()` path is used as a fallback if the direct server connection fails. This dramatically reduces reconnect latency for orphaned nodes — they get data flowing again in milliseconds instead of waiting for Firebase round-trips.
