...
When root sees `serverUrl` in Firebase config (relay server is online), it sets K=0 via `_onconfig`. Since the server gets a free slot (above), root accepts only the relay server as a child — all regular peers are funneled through the relay. This preserves root's bandwidth for broadcasting. When `serverUrl` disappears (server offline), root restores K to the user-configured value so peers can form a direct P2P tree. The user-set K is tracked in `_baseK` separately from `opts.K` since `deepMerge` overwrites `opts.K` on every config change.
