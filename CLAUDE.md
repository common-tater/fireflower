...
When root sees `serverUrl` in Firebase config (relay server is online), it sets K=0 via `_onconfig`. Combined with the server's free slot, root accepts only the relay server — all regular peers are funneled through the relay. This preserves root's bandwidth for broadcasting. One server-connected node will typically remain on server transport permanently: as other nodes upgrade to P2P via `_attemptUpgrade`, they form a descendant chain. The last node on server can't upgrade because all potential targets are its own descendants (circle prevention blocks it). This is correct behavior — the relay server always has at least one direct child, and that node serves as a healthy sub-tree root. When `serverUrl` disappears (server offline), root restores K to the user-configured value so peers can form a direct P2P tree. The user-set K is tracked in `_baseK` separately from `opts.K` since `deepMerge` overwrites `opts.K` on every config change.
